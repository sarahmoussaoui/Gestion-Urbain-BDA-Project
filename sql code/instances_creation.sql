...
INSERT INTO StationTab VALUES (
    StationPrincipaleType(
        'SP001', 'Ha� El Badr', 3.080000, 36.750000,
        TabMoyenTransport(
...
INSERT INTO StationTab VALUES (
    StationSecondaireType(
        'SS001', 'Les Fusill�s', 3.090000, 36.755000,
        (SELECT REF(m) FROM MoyenTransportTab m WHERE m.code_transport = 'M')
    )
);
COMMIT;
...
INSERT INTO StationTab VALUES (
    StationSecondaireType(
        'SS006', 'Sidi Bel Abb�s', 3.180000, 36.690000,
...
INSERT INTO LigneTab VALUES( 
    LigneType(
        ...
        NULL   -- Liste des tron�ons
    )
);
COMMIT;


-- Insertion de la ligne de m�tro
INSERT INTO LigneTab VALUES( 
    LigneType(
        'M001', 
        (SELECT REF(m) FROM MoyenTransportTab m WHERE m.code_transport = 'M'), 
...
-- insertion des tron�ons
-- Tron�ons pour le M�tro
INSERT INTO TronconTab VALUES (
    TronconType(101,
        (SELECT REF(s) FROM StationTab s WHERE s.code_station = 'SP001'),  -- Ha� El Badr
        (SELECT REF(s) FROM StationTab s WHERE s.code_station = 'SS001'),  -- Les Fusill�s
        1.50,
...
-- Tron�ons pour le Train
INSERT INTO TronconTab VALUES (
    TronconType(401,
        (SELECT REF(s) FROM StationTab s WHERE s.code_station = 'SS003'),  -- Bab Ezzouar
        (SELECT REF(s) FROM StationTab s WHERE s.code_station = 'SS006'),  -- Sidi Bel Abb�s
        6.50,
